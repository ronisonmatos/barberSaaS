-- Cobranca real via Pix (Mercado Pago). Estabelecimento escolhe quais formas aceita
-- (pagar ao agendar e/ou pagar no dia) e guarda o webhook secret da propria conta Mercado Pago.

alter table estabelecimento_pagamento_config
  add column aceita_pagamento_antecipado boolean not null default false,
  add column aceita_pagamento_no_dia boolean not null default true,
  add column mercado_pago_webhook_secret text;

-- RPC publica: cria agendamento PENDENTE + pagamento PENDENTE (pix), na mesma transacao.
-- Diferente de criar_agendamento_publico (que ja confirma direto, fluxo no_local).
-- Exige e-mail do cliente porque o Mercado Pago exige e-mail do pagador pra cobranca Pix.
...
begin
  ...
  values (p_estabelecimento_id, v_agendamento_id, v_cliente_id, v_servico.preco_centavos, 'pix', 'pendente')
...
grant execute on function criar_agendamento_publico_pix(uuid, uuid, uuid, timestamptz, text, text, text) to anon, authenticated;
