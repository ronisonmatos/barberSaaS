-- Generaliza criar_agendamento_publico_pix para tambem aceitar cartao de credito.
-- Adicionar parametro via CREATE OR REPLACE criaria uma sobrecarga (funcao com 7
-- parametros continuaria existindo ao lado da nova de 8) -- dropamos explicitamente
-- pra manter uma unica versao.

drop function if exists criar_agendamento_publico_pix(uuid, uuid, uuid, timestamptz, text, text, text);
...
grant execute on function criar_agendamento_publico_pix(uuid, uuid, uuid, timestamptz, text, text, text, metodo_pagamento) to anon, authenticated;
