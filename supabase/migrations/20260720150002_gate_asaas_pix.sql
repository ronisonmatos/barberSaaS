-- Libera Pix tambem via gateway Asaas (ate aqui so Mercado Pago era aceito). Cartao continua
-- exclusivo da Mercado Pago -- a Asaas nao tem integracao de cartao nesta leva. Assinatura nao
-- muda (create or replace), so a condicao do gate de gateway.
...
begin
  ...
  if not found or not v_config.aceita_pagamento_antecipado
     or v_config.gateway_ativo = 'nenhum'
     or (p_metodo = 'cartao' and v_config.gateway_ativo <> 'mercado_pago')
  then
    raise exception 'pagamento antecipado indisponivel para este estabelecimento';
  end if;
