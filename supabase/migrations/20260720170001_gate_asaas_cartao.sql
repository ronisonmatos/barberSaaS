-- Libera cartao tambem via gateway Asaas (ate aqui so Mercado Pago). O cliente final agora pode
-- pagar cartao parcelado via Checkout hospedado da Asaas (ver web/lib/asaas.ts::criarCheckoutAsaas),
-- entao a restricao "cartao so com mercado_pago" nao faz mais sentido -- os dois gateways aceitos
-- (gateway_ativo <> 'nenhum') ja e suficiente. Mesmas 3 funcoes, create or replace, so o gate muda.
