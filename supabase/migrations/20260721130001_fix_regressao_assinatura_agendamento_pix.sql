-- Bug real encontrado ao reexecutar supabase/tests/clube_assinatura.sql desta leva: a migration
-- 20260720170001_gate_asaas_cartao.sql (integracao Asaas) recriou criar_agendamento_publico_pix a
-- partir de uma versao anterior a 20260720100004_consumo_assinatura_no_agendamento.sql, perdendo
-- silenciosamente toda a logica de consumo do clube de assinatura (assinatura_disponivel_para_servico,
-- confirmacao automatica sem cobranca, incremento de usos_ciclo). Na pratica, em producao, um
-- cliente com assinatura ativa e cota livre que pagasse via Pix/cartao (fluxo criar_agendamento_publico_pix)
-- era cobrado normalmente em vez de ter o agendamento coberto pela assinatura -- so o fluxo sem
-- pagamento antecipado (criar_agendamento_publico) continuava correto, ja que nao foi tocado por
-- aquela migration.
--
-- Esta migration reaplica a logica de consumo (identica a 20260720100004) por cima da versao mais
-- recente da funcao (gate_asaas: gateway_ativo <> 'nenhum' em vez de = 'mercado_pago').
