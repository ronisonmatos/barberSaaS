...
import { ServicoFormRascunho } from "./servico-form-rascunho";
import { alternarAtivoServicoRascunho } from "./actions";
...
export function ServicosRascunhoClient({
  estabelecimentoId,
  servicos,
}: {
            ...
            <tbody>
              {servicos.map((s) => (
                <tr key={s.id} className="border-b border-linha text-carvao hover:bg-marfim">
                  ...
                  <td className={s.ativo ? "text-sucesso" : "text-cinza-600"}>{s.ativo ? "Ativo" : "Inativo"}</td>
                  <td className="flex gap-2 py-2 text-right">
                    ...
                    <Button
                      variant="ghost"
                      disabled={isPending}
                      onClick={() => startTransition(() => alternarAtivoServicoRascunho(estabelecimentoId, s.id, !s.ativo))}
                    >
