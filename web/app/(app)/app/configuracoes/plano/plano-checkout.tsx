...
export function PlanoCheckout({
  ...
  useEffect(() => {
    ...
    const intervalo = setInterval(async () => {
      const status = await statusPagamentoPlano(pendente);
  ...
  if (confirmado) {
    return (
      <div className="flex flex-col items-center gap-2 rounded-md border border-linha bg-marfim p-4 text-center">
        <p className="font-medium text-carvao">Assinatura confirmada!</p>
        <p className="text-sm text-cinza-600">Plano {planoNome} ativado.</p>
        <Button
          onClick={() => {
            router.refresh();
            onFechar();
  ...
  if (passo === "pix" && pix) {
    return (
      <div className="flex flex-col items-center gap-3 rounded-md border border-linha bg-marfim p-4 text-center">
        <p className="font-medium text-carvao">Pague com Pix para ativar o plano {planoNome}</p>
        ...
        {cpfValido && (
          <CardPaymentBrick
            ...
            onEnviar={async (formData) => {
              const r = await criarCobrancaCartaoPlano({ planoId, cpf, formData });
        ...
        <Button
          ...
          onClick={() => {
            ...
            startTransition(async () => {
              const r = await criarCobrancaPixPlano({ planoId });
